...
// Canvas-based force graph — GPU-composited draw, handles 700+ nodes at 60fps
// Spring-physics drag, glow via canvas shadowBlur, timelapse pop-in animation
// Adapted from the Quickbooks KnowledgeGraph for GA Real Estate Knowledge Brain
...
/* ─── Category palette (GA Real Estate topics) ───────────────────────── */
...
function getNodeRadius(node: BrainNode): number {
  ...
  return Math.max(5, Math.min(20, 5 + connections * 1.4));
}
...
export function KnowledgeBrainGraph({ nodes: rawNodes, edges: rawEdges, onSelectNote, onBack }: Props) {
  ...
  // Enrich nodes with link counts
  const enrichedData = useCallback(() => {
    const linkCountMap = new Map<string, number>();
    const backlinkCountMap = new Map<string, number>();
    for (const e of rawEdges) {
      linkCountMap.set(e.sourceId, (linkCountMap.get(e.sourceId) ?? 0) + 1);
      backlinkCountMap.set(e.targetId, (backlinkCountMap.get(e.targetId) ?? 0) + 1);
    }
    const nodes: BrainNode[] = rawNodes.map(n => ({
      ...n,
      linkCount: linkCountMap.get(n.id) ?? 0,
      backlinkCount: backlinkCountMap.get(n.id) ?? 0,
    }));
  ...
  const draw = useCallback(() => {
    ...
    for (const node of nodes) {
      ...
      if (hovered && hovered.id === node.id && !selId) {
        ctx.beginPath();
    ...
    if (k >= LABEL_ZOOM_THRESHOLD) {
      const fontSize = Math.max(8, Math.min(12, 9 / k));
      ...
      for (const node of nodes) {
        ...
        ctx.fillStyle = node.id === selId ? "#ffffff" : `rgba(255,255,255,${isConnected ? 0.7 : 0.12})`;
        const label = node.title.length > 22 ? node.title.slice(0, 22) + "…" : node.title;
        ctx.fillText(label, node.x ?? 0, (node.y ?? 0) + getNodeRadius(node) + 12 / k);
      }
    }
  ...
  useEffect(() => {
    ...
    const { nodes: enrichedNodes, edges } = enrichedData();
    ...
    import("d3").then((d3) => {
      ...
      const visibleNodes = activeCategory
        ? enrichedNodes.filter(nd => nd.category === activeCategory)
        : [...enrichedNodes];
      ...
      let panStartTransform = { x: 0, y: 0, k: 1 };
      ...
      simulation.on("tick", () => { /* RAF loop handles drawing */ });

      // ── Entrance animation — hub-first cascade ──────────────────────
  ...
  }, [rawNodes, rawEdges, activeCategory, enrichedData, getNodeAtPoint, startLoop, stopLoop]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-12 left-3 z-10 flex flex-col" style={{ maxHeight: "calc(100% - 5rem)" }}>
        <div className="flex items-center gap-1 text-[10px] text-gray-600 mb-1 flex-shrink-0">
          <Filter className="w-3 h-3" /> Filter
        </div>
        <div className="flex flex-col gap-1 overflow-y-auto scrollbar-hide">
          <button
            onClick={() => setActiveCategory(null)}
            className={`graph-filter-btn ${!activeCategory ? "graph-filter-active" : ""}`}
          >
            All
          </button>
          {categories.map(cat => (
      ...
      <div className="absolute top-3 right-3 flex flex-col gap-1.5 z-10">
        <button onClick={handleZoomIn} className="graph-control-btn" title="Zoom In"><ZoomIn className="w-3.5 h-3.5" /></button>
        <button onClick={handleZoomOut} className="graph-control-btn" title="Zoom Out"><ZoomOut className="w-3.5 h-3.5" /></button>
        <button onClick={handleZoomFit} className="graph-control-btn" title="Zoom to Fit"><Crosshair className="w-3.5 h-3.5" /></button>
        <button onClick={startLoop} className="graph-control-btn" title="Refresh"><RefreshCw className="w-3.5 h-3.5" /></button>
        <button onClick={() => setFullscreen(f => !f)} className="graph-control-btn" title="Toggle Fullscreen">
          {fullscreen ? <Minimize2 className="w-3.5 h-3.5" /> : <Maximize2 className="w-3.5 h-3.5" />}
        </button>
      </div>

      {/* Stats */}
      <div className="absolute top-48 right-3 text-[10px] text-gray-700 z-10 text-right">
        {rawNodes.length} nodes<br />{rawEdges.length} links<br />{Math.round(zoom * 100)}%
      </div>
      ...
      {selectedNodeData && !timelapsePlaying && (
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-[#0d1526]/95 border border-re-500/40 rounded-xl px-4 py-2.5 flex items-center gap-3 backdrop-blur-sm shadow-2xl z-10 min-w-[280px] max-w-md">
          <div
            ...
            style={{ backgroundColor: CATEGORY_COLORS[selectedNodeData.category] ?? CATEGORY_COLORS.General }}
          />
      ...
      {hoveredNode && (
        ...
        >
          ...
          <div className="text-gray-600 mt-1">
            {hoveredNode.linkCount ?? 0} links · {hoveredNode.backlinkCount ?? 0} backlinks
          </div>
          {!selectedNodeData && (
      ...
      {/* Category legend — bottom right above timelapse */}
      <div
